      ...
      <aside className={cn("fixed md:static inset-y-0 left-0 z-40 flex flex-col w-64 shrink-0", "border-r border-zinc-200 dark:border-zinc-800/80", "bg-white dark:bg-[#0c0c0e]/95 backdrop-blur-md", "transition-transform duration-300 md:translate-x-0", sidebarOpen ? "translate-x-0" : "-translate-x-full")}>
        <div className="h-16 flex items-center justify-between px-6 border-b border-zinc-200 dark:border-zinc-800/80">
          <Link href="/" className="flex items-center gap-2.5">
            <div className="h-8 w-8 rounded-lg bg-gradient-to-tr from-indigo-600 to-violet-500 flex items-center justify-center text-white shadow-md shadow-indigo-500/20"><Activity className="size-4" /></div>
            <span className="font-bold text-lg tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-zinc-900 via-zinc-800 to-zinc-600 dark:from-white dark:to-zinc-400">{user?.company || "RailSwitch"}</span>
          </Link>
        ...
        <nav className="flex-1 px-4 py-5 space-y-0.5 overflow-y-auto">
          {NAV_ITEMS.map((item) => (
            <Link key={item.href} href={item.href} onClick={() => setSidebarOpen(false)} className={cn("flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200", isActive(item.href) ? "bg-zinc-100 dark:bg-zinc-800/80" : "text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100 hover:bg-zinc-50 dark:hover:bg-zinc-800/30")}>
        ...
        <div className="p-4 border-t border-zinc-200 dark:border-zinc-800/80">
          <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors duration-200">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold text-zinc-900 dark:text-zinc-100 truncate">{user?.name ?? "User"}</p>
              <p className="text-[10px] text-zinc-500 dark:text-zinc-400 truncate">{user?.email ?? ""}</p>
            </div>
            <button onClick={logout} className="p-1 rounded text-zinc-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/30 transition-colors" aria-label="Sign out"><LogOut className="size-3.5 shrink-0" /></button>
          </div>
        </div>
      </aside>
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        <header className="h-16 flex items-center justify-between py-4 px-4 md:px-8 border-b border-zinc-200 dark:border-zinc-800/80 bg-white/70 dark:bg-[#0c0c0e]/70 backdrop-blur-md sticky top-0 z-20">
          ...
          <div className="flex items-center gap-3 ml-auto">
            <button className="relative p-1.5 rounded-lg text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 hover:bg-zinc-100 dark:hover:bg-zinc-800/50 transition-colors" aria-label="Notifications" onClick={() => { setNotificationsOpen(true); setHasNewNotifications(false); }}>
            ...
            <Link href="/dashboard/plans" className="inline-flex items-center gap-1.5 h-8 px-3 rounded-lg text-xs font-semibold bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white shadow-sm shadow-indigo-500/20 transition-all">
              <span className="hidden sm:inline">New Plan</span><span className="sm:hidden">New</span><ArrowUpRight className="size-3.5" />
            </Link>
          </div>
        </header>
        <main className="flex-1 p-4 md:p-8 max-w-7xl w-full mx-auto animate-in fade-in-50 slide-in-from-bottom-2 duration-500">{children}</main>
      </div>
      <Sheet open={notificationsOpen} onOpenChange={setNotificationsOpen}>
        <SheetContent side="right" className="w-full sm:max-w-md bg-white dark:bg-[#111113] border-l border-zinc-200 dark:border-zinc-800/80 p-0 overflow-y-auto flex flex-col">
          <SheetHeader className="px-6 py-5 border-b border-zinc-100 dark:border-zinc-800/60">
            <SheetTitle className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Notifications</SheetTitle>
            <SheetDescription className="text-xs text-zinc-500 dark:text-zinc-400">Recent activity on your account.</SheetDescription>
          </SheetHeader>
          <div className="flex-1 overflow-y-auto">
            {notifications.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
                ...
                <p className="text-xs text-zinc-400 mt-1">New subscriptions and recovery events will appear here.</p>
              </div>
            ) : (
                ...
                {notifications.map((n) => (
                  <div key={n.id} className="px-6 py-4 hover:bg-zinc-50 dark:hover:bg-zinc-800/20 transition-colors">
                    <div className="flex items-start gap-3">
                      <div className={`size-8 rounded-lg flex items-center justify-center shrink-0 mt-0.5 ${n.type === 'subscription' ? 'bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600' : 'bg-amber-50 dark:bg-amber-950/30 text-amber-600'}`}>
